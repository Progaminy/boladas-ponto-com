...
from app.config import APP_NAME, APP_VERSION, SESSION_SECRET_KEY, SESSION_COOKIE_SECURE, b2_configured
...
from app.routers import auth, business, compare, explore, history, media, messages, moderation, posts, profile, transactions
...
async def lifespan(app: FastAPI):
    if database_backend_name() == "sqlite":
        ...
        try:
            ...
            print(f"DATABASE_STARTUP_CHECK=failed {type(exc).__name__}: {str(exc)[:500]}", flush=True)
    yield
...
def database_health():
    try:
        ...
        return JSONResponse(status_code=503, content={"ok": False, "database_backend": database_backend_name(), "error": str(exc)[:500]})

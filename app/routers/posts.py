...
def create_form(request: Request):
    ...
    return templates.TemplateResponse(
        request,
        "create.html",
        {"categories": list_categories(), "businesses": businesses},
    )
...
def create_post(
    ...
    if recent_count >= MAX_POSTS_PER_USER_PER_DAY:
        return JSONResponse(
            {
                "error": (
                    f"Limite de {MAX_POSTS_PER_USER_PER_DAY} anúncios por dia atingido. "
    ...
    final_theme = (theme or "").strip() or business.strip()
    ...
    try:
        post_input = PostInput(
            ...
            publisher_type=(
                PublisherType.BUSINESS if selected_business_id else PublisherType.INDIVIDUAL
            ),
            ...
            tone="manual",
            language="pt",
            call_to_action="Contacta o vendedor",
            price_mt=price_mt,
            ...
            description_source="manual" if (description or "").strip() else None,
        )
    ...
    blocked_terms = check_text_blocklist(
        post_input.theme,
        post_input.business,
        post_input.description or "",
    )
    if blocked_terms:
        return JSONResponse(
            {"error": "Conteúdo não permitido pelos Termos de Uso. Revê o anúncio."},
            status_code=422,
        )
    ...
    return JSONResponse({"post_id": post_id, "status": PostStatus.COMPLETED.value})
